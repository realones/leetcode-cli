...
plugin.getSubmissions = function(problem, cb) {
  log.debug('running lintcode.getSubmissions');
  // yji: use makeOpts to get cookie, don't use plugins.makeOpts
  const opts = makeOpts(config.sys.urls.submissions.replace('$id', problem.id));
  request(opts, function(e, resp, body) {
    e = checkError(e, resp, 200);
    if (e) return cb(e);

    // FIXME: this only return the 1st 20 submissions, we should get next if necessary.
    const submissions = JSON.parse(body).submissions;

    for (let submission of submissions){
      //translate to enable submission.js -> exportSubmission to use it
      if (submission.language.toUpperCase() === 'C++'.toUpperCase()){
        submission.lang = 'cpp';
      }

      if (submission.language.toUpperCase() === 'java'.toUpperCase()){
...
plugin.getSubmission = function(submission, cb) {
  log.debug('running lintcode.getSubmission');
  ...
  request(opts, function(e, resp, body) {
    ...
    submission.code = JSON.parse(body).code;
