...
cmd.handler = function(argv) {
  ...
  core.filterProblems(argv, function(e, problems) {
    ...
    if (word) {
      ...
      {
        problems = problems.filter(x => x.name.toLowerCase().includes(words[i]));
      }
      // problems = problems.filter(x => x.name.toLowerCase().includes(word));
      
      //breaks the other possible parameters, need to fix in future
      for(let i=1;i<argv._.length;++i)
      {
        problems = problems.filter(x => x.name.toLowerCase().includes(argv._[i]));
      }
    }
